...
import {
  initialAdmissionData, type AdmissionApplication, type AdmissionDocument, type AdmissionFormData,
  type AdmissionSection, type EmergencyContact, type Guardian,
...
function TrackModal({ onClose }: { onClose: () => void }) {
  const [form, setForm] = useState({ applicationNumber: "", email: "", dateOfBirth: "" });
  ...
  useEffect(() => {
    ...
    try {
      ...
      getAdmissionApplication(draft.id, draft.token).then((result) => {
        ...
        setData({ ...initialAdmissionData, ...result.formData } as AdmissionFormData);
